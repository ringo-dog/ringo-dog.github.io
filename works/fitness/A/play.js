import{beforebegin,db,funBack,inSec,inTime,none}from'./function.js'
...
counts=['10p','9p','8p','7p','6p','5p','4p','3p','2p'],
...
clear=()=>{clearInterval(interval);try{clearTimeout(timeout)}catch{}},
  ...
  let fun=()=>funEnd(()=>{funVoice('end');history.back()})
...
funTimeout=()=>{let fun=()=>funVoice(time);setTimeout(fun,2000);setTimeout(fun,1000);fun()},
...
show=(ar)=>{
  ...
  clear();if(T3.querySelector('td.shadow-sm:first-child.d-none')){interval=setInterval(minus,1000)}funVoice(ar[0])
},
funVoice=(name)=>{audio.src=`../voice/${name}.mp3`;audio.play()},
rest=[
...
work=[
  ()=>{
    ...
    imgs[0].src=imgs[1].src;work[3]()
    if(ar[0]===0)return work[2]()
    next=rest[0];funImg();count=counts.slice(-ar[0])
    if(count.length<ar[0]){let size=ar[0]-count.length,i=0;for(i;i<size;i++)count.unshift(null)}
  },
  ()=>{work[3]();if(count[ar[0]])funVoice(count[ar[0]])/* перекрывает */;if(ar[0]===0){work[2]()}else{next=rest[0]}},
...
arr.forEach(ar=>ar[5]=links[ar[5]])
links=links[0]
ar=arr.shift()
funImg()
route()
